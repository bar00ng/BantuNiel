...
<script>
    ...
    // Filter
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // OLT
            const bigOltCheckbox = document.getElementById("bigOltCheckbox");
            const miniOltCheckbox = document.getElementById("miniOltCheckbox");
            // Witel
            ...
            function applyFilter() {
                rows.forEach(function(row) {
                    ...
                    const merkCell = row.querySelector("td:nth-child(4)").textContent;
                    const oltCell = row.querySelector("td:nth-child(7)").textContent;
                       // Witel
                    ...
                    // OLT SIZE
                    const isBig = bigOltCheckbox.checked;
                    const isMini = miniOltCheckbox.checked;

                    if ((isBdg && witelCell === "BANDUNG") || (isBdb && witelCell === "BANDUNG BARAT") || (isCbn && witelCell === "CIREBON") || (isKrw && witelCell === "KARAWANG") || (isSkb && witelCell === "SUKABUMI") || (isTsm && witelCell === "TASIKMALAYA") || (isFiber && merkCell === "FH") || (isHuawei && merkCell === "HWE") || (isZte && merkCell === "ZTE") || (isBig && oltCell === "BIG") || (isMini && oltCell === "MINI") ) {
                        row.style.display = "";
                    } else if (!isFiber && !isHuawei && !isZte && !isBig && !isMini && !isBdg && !isBdb && !isCbn && !isKrw && !isSkb && !isTsm) {
            ...
            // OLT
            bigOltCheckbox.addEventListener("change", applyFilter);
            miniOltCheckbox.addEventListener("change", applyFilter);
            // Witel
